...
export default function Dashboard() {
  ...
  return <div className="page-transition admin-page">
    <header className="admin-page-heading"><div><span className="admin-eyebrow">{company.name} · Plano {company.plan}</span><h1>Visão geral</h1><p>Acompanhe as rotas e os passageiros de hoje.</p></div></header>
    ...
    <section className="admin-metric-grid" aria-label="Indicadores da operação">
      ...
      <article className="card admin-metric admin-metric-routes"><div><span>Rotas cadastradas</span><RouteIcon size={21} /></div><strong>{summary.routes.total}</strong><div className="admin-route-breakdown"><span><Clock3 size={14} />{summary.routes.inProgress} em andamento</span><span><CheckCircle2 size={14} />{summary.routes.completed} concluída</span><span><CircleDashed size={14} />{summary.routes.planned} aguardando</span></div></article>
      <article className="card admin-metric"><div><span>Ocorrências e atrasos</span><AlertTriangle size={21} /></div><strong>{summary.occurrences}</strong><small>Registrados nas rotas</small></article>
    </section>
    ...
    <div className="admin-dashboard-grid">
      ...
      <aside className="card admin-insights"><span className="admin-eyebrow">Resumo do dia</span><h2>O que merece atenção</h2><div><strong>{critical.length}</strong><span>rotas com baixa ocupação</span></div><div><strong>{noShows}</strong><span>ausências nas rotas nos últimos 30 dias</span></div><p>Selecione rotas críticas para comparar uma possível reotimização ou unificação. A prévia não altera a operação.</p><Link className="btn btn-primary" to="/company/routes">Explorar todas as rotas</Link></aside>
    </div>
